...
#define SCRIBER_VIEW_H 1064
#define SCRIBER_MAX_ROWS 600
...
static int scriber_row_starts[SCRIBER_MAX_ROWS];
static int scriber_row_count = 0;
static int scriber_top_row = 0;
...
static void scriber_build_rows(void) {
    scriber_row_count = 0;
    scriber_row_starts[scriber_row_count++] = 0;
    int x = 1;
    for (int i = 0; i < scriber_len; i++) {
        char c = scriber_buf[i];
        if (c == '\n') {
            x = 1;
            if (scriber_row_count < SCRIBER_MAX_ROWS) scriber_row_starts[scriber_row_count++] = i + 1;
            continue;
        }
        x += 8;
        if (x > 1920 - 8) {
...
static int scriber_row_of(int idx) {
    int row = 0;
    for (int i = 0; i < scriber_row_count; i++) {
        if (scriber_row_starts[i] <= idx) row = i;
        else break;
    }
    return row;
}

static void scriber_adjust_scroll(void) {
    int max_rows = SCRIBER_VIEW_H / 8;
    int cur_row = scriber_row_of(scriber_pos);
    if (cur_row < scriber_top_row) {
        scriber_top_row = cur_row;
    }
    if (cur_row > scriber_top_row + max_rows - 1) {
        scriber_top_row = cur_row - max_rows + 1;
    }
    if (scriber_top_row < 0) scriber_top_row = 0;
    if (scriber_top_row > scriber_row_count - 1) scriber_top_row = scriber_row_count - 1;
}

static void scriber_redraw(void) {
    scriber_build_rows();
    scriber_adjust_scroll();
    vesa_draw_rec(0, 0, 1920, SCRIBER_VIEW_H, 0x000000);
    int start = scriber_row_starts[scriber_top_row];
    int x = 1, y = 1;
    int cur_x = 1, cur_y = 1;
    for (int i = start; i <= scriber_len; i++) {
        if (i == scriber_pos) {
            cur_x = x;
            cur_y = y;
        }
